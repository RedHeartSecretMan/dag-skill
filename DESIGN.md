...
The open Agent Skills frontmatter exposes `name: dag` and a focused description for requests to execute, advance, continue, or resume an approved multi-Ticket DAG.
...
The execution profile uses complete directories named `implement`, `code-review`, `tdd`, and `codebase-design`, pinned to `mattpocock/skills` revision `5b15a47f2d7150f545fbcacbfe381787fc0230dc`. The setup helper reads each directory from the upstream `skills/engineering/` grouping and installs it directly under the explicit host Skills root.
...
- bundle, Ticket, attempt, ownership, and budget state;
...
Every directly dispatched Agent receives a bounded progress checkpoint. Evidence-bearing milestones, evidenced blockers, and terminal results demonstrate progress; any other outcome at the bound produces an Operational Failure.

## Per-Ticket flow

### 1. Fixed point and implementation

Immediately before claim, the Coordinator rereads the Ticket, dependencies, blockers, repository, tracker, and bundle identities. It claims the Ticket, records an immutable Review Fixed Point, and dispatches one fresh Execution Agent in a candidate workspace that preserves the Authoritative Integration Baseline.

An applicable `implement` dispatch receives the fixed point, Spec, Ticket, standards, tracker context, scope, and authorization. Its nested `tdd`, conditional `codebase-design`, and `code-review` uses resolve from the same bundle. The nested Standards and Spec review forms Implementation-Side Review evidence.
...
The Coordinator checks the live candidate, public delivery seam, verification, reviewer independence, artifact identity, and generated or lockfile changes. Acceptance requires green gates or the exact Target Project-permitted exception with an owner and closing condition.

One Operational Retry is shared across implementation, review, and integration for each Ticket attempt. The Coordinator persists its use and redispatches the appropriate role. A later operational failure blocks the Ticket.
...
The exact reviewed candidate then enters the Authoritative Integration Baseline. The Coordinator proves artifact representation or reachability and runs affected, integration, and delivered-public-seam gates on the integrated bytes. Byte changes return through the Review Sufficiency Gate.
...
Each Ticket has one Formal Rework for an accepted blocking review or integration finding within its original scope. The reworked artifact repeats the complete Ticket flow. Self-correction before handoff, duplicate findings, false positives, and Operational Retry remain outside this budget.
...
A valid late finding or Whole-DAG failure invalidates the contradicted Ticket and every accepted descendant that consumes its output. The affected subgraph resumes through remaining Formal Rework or an authoring-produced remediation lineage.

...
6. On bundle failure, stop before claiming or dispatching any Ticket and report the exact repair required. Do not convert absence into DAG-Native Fallback, consume an Operational Retry, or install, register, update, or overwrite user Skills under DAG Run Authorization. After an authorized out-of-run repair, reread and revalidate the whole bundle. Keep `setup-matt-pocock-skills` outside the bundle; do not install or activate it under DAG Run Authorization because it configures the Target Project.

...
6. On bundle failure, stop before claiming or dispatching any Ticket and report the exact repair required. Do not convert absence into DAG-Native Fallback, consume an Operational Retry, or install, register, update, or overwrite user Skills under DAG Run Authorization. If the user separately authorizes dependency installation and the Agent Host's stable Skill directory is explicit, run `scripts/install_dependencies.py --destination <skill-directory>` relative to this Skill; the setup helper installs only the pinned bundle and refuses conflicting targets. If Python, Git, network access, or an explicit destination is unavailable, report the manual repair instead of guessing. After any authorized out-of-run repair, reread and revalidate the whole bundle. Keep `setup-matt-pocock-skills` outside the bundle; do not install or activate it under DAG Run Authorization because it configures the Target Project.
